...
void main()
{
    ...
    vec4 textColor = texture2D(texture, uv.st);
    ...
    else
    {
        for(int i=0; i<lightCount; i++)
        {
            ...
            dfColor += vertColor.rgb * textColor.rgb * lightDiffuse[i] * intensity;
            spColor += lightSpecular[i] * spec;
            amColor += lightAmbient[i];
        }

        gl_FragColor = vec4(dfColor + amColor + spColor, vertColor.a * textColor.a);
    }
}
